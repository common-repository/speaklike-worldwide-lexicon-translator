...
class WordpressWWLController extends WWLController {
	...
	function getLanguageConfig($lang) {
		...
		$config["wwl_scoring_role"] = get_option('wwl_scoring_role');
	...
	function registerWpOptions() {
		...
		register_setting('wwl-options', 'wwl_scoring_role');
	...
	function canTranslate() {
		return $this->hasAccess($this->config["vol"]);
	}
	function canScore() {
		return $this->hasAccess($this->config['wwl_scoring_role']);
	}
	function listRoles() {
		$role = new WP_Roles();
		$out = array(

...
class WWLController {
	...
	function saveToCache($st, $tt, $ttl = 0){
		...
		$data = $tt."|".$st."|".$this->sourceLanguage."|".$this->targetLanguage."|".$this->meta["username"]."|".$this->meta["mtengine"]."|".$this->meta["guid"];
		foreach ($this->cacheProviders as $cacher){
		...
		foreach ($this->cacheProviders as $cacher){
			if ($res = $cacher["worker"]->get($key)) {
				...
				$this->meta = array(
					...
					"guid" => isset($res[6]) ? $res[6] : "",
					"id" => $this->contentItemId
				);
				return $res[0];
			}
		}
		$this->meta = array("mtengine" => "google", "username" => "", "guid" => "", "id" => $this->contentItemId);
		return "";
	}	
	...
	function updateTranslation($st, $tt){
		...
		foreach ($this->translationProviders as $translator){
			...
			$this->meta["guid"] = $translator["worker"]->guid;
		}
		$this->saveToCache($st, $tt);
		return $res;
	}
	function scoreTranslation($st, $tt, $guid, $score) {
		$this->log("Rating translation");
		$this->log("Source: ".$st);
		$this->log("Translated: ".$tt);
		$this->log("Guid: ".$guid);
		$this->log("Score: ".$score);
		if ($score === 0) {
			$this->log("Reset cached value");
			$this->saveToCache($st, "");
		}
		foreach ($this->translationProviders as $translator){
			if (!method_exists($translator["worker"], "score")){
				continue;
			}
			$res = $translator["worker"]->score($guid, $score, $this->config);
			$this->log("url: ".$translator["worker"]->lastUrl." ".$translator["worker"]->error);
		}
		return $res;
	}

...
class WWLAPI extends BaseTranslator {
		...
		if ( strlen($this->sourceLanguage) > 0 && strlen($this->targetLanguage) > 0 && strlen($st) > 0 ) {
			...
			if ($response) {
				...
				if ($response && $response["records"] && $response["records"][0] && $response["records"][0]["tx"]){
					...
					$this->meta["guid"] = $response["records"][0]["tx"]["guid"];
	 ...
	 *
	 *  @param string $guid -- translation GUID as returned by service
	 *	@param string $votetype 
	 *  @param string $username
	 *  @param string $pw
	 *	@return bool
	 */
	function score( $guid, $score=0, $username='', $pw='') {
		if (!$guid) {
			$this->error = "Empty guid";
			return "";
		}

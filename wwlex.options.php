...
<table class="form-table">
<tr valign="top">
<th scope="row"><label for="wwl_scoring_role"><?php _e('User role required to score transaltions:') ?></label></th>
<td><select name="wwl_scoring_role" id="wwl_scoring_role">
<?php
	$line = "";				
	foreach ($wwlex->listRoles() as $role => $name) {
		$selected = $role == $wwlex->config["wwl_scoring_role"] ? 'selected' : '';
		$line.= '<option value="'.$role.'" '.$selected.'>'.$name.'</option>';
	}
	echo $line;
?>
...
if (!function_exists('register_setting')) {
?>
<input type="hidden" name="page_options" value="wwl_scoring_role,wwl_hook_gettext,wwl_hide_decorations,wwl_translation_timeout,wwl_cache_timeout,wwl_language,speaklike_user,speaklike_pass,<?php echo implode(",", $permOptions);?>" />
<?php } ?>
<p class="submit">

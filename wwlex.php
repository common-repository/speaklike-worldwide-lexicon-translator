...
function wwl_footer() {
	...
	echo 'wwl.canScore = '.($wwlex->canScore() ? "true" : "false").';';
...
function wwl_rate() {
	...
	$out = array();
	$out["chunkId"] = $_POST["chunkId"]; // echo back
	$out["result"] = $wwlex->scoreTranslation($_POST["source"], $_POST["translated"], $_POST["guid"], intval($_POST["score"]));
	die(str_replace("\\\\", "\\", $wwlex->jsonEncode($out)));
}
...
add_action('wp_ajax_wwl_rate', 'wwl_rate');
add_action('wp_ajax_nopriv_wwl_rate', 'wwl_rate');
